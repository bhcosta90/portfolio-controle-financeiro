...
$actions = [
    ...
    'executeCaptainHook' => 'Creating the captain hook',
];
...
function executePintPreparation(): bool|string
{
    try {
        ...
        $content = (new Client())->get(
            'https://gist.githubusercontent.com/bhcosta90/bd0fe7f7fa723ee68078ed9c5970742d/raw/0e79e81fb89bb80b225402b954f6983c07de505b/pint.json'
        );
...
function executeCaptainHook(): bool|string
{
    ...
    foreach ($executionSteps as $step) {
        switch ($step) {
            ...
            case 'pint':
                ...
                break;
            case 'pest':
                $actions[] = "./vendor/bin/pest --parallel";
    ...
    foreach ($actions as $action) {
        $content .= "{\"action\": \"{$action}\",\"options\": []},\n";
    }

    $content = substr($content, 0, -2);

    //    if (executeCommand("composer require captainhook/captainhook --dev") !== true) {
    //        return false;
    //    }
