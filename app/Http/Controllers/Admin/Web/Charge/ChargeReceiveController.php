...
class ChargeReceiveController extends Controller
{
    ...
    public function payShow(FormSupport $formSupport, string $id, Services\FindService $find)
    {
        ...
        $formPartial = $formSupport->run(PartialForm::class, route('admin.charge.receive.pay.partial.store', $id));
        $formTotal = $formSupport->run(TotalForm::class, route('admin.charge.receive.pay.total.store', $id));
        return view('admin.charge.receive.pay', compact('formPartial', 'formTotal', 'model'));
    }
