...
namespace App\Http\Controllers\Web\Admin\Relationship;
...
use Core\Application\Relationship\Modules\Company\UseCases;
...
{
    private $title = [
        'store' => 'Empresa cadastrada com sucesso',
        'update' => 'Empresa editada com sucesso',
        'destroy' => 'Empresa deletada com sucesso'
    ];

    private $route = 'admin.relationship.company';
    ...
    public function store(FormSupport $formSupport, UseCases\CreateUseCase $createUseCase)
    {
        ...
        $model = $createUseCase->handle(new UseCases\DTO\Create\Input(auth()->user()->tenant_id, $data['name']));
    ...
    {
        $data = $formSupport->data(Form::class);
        $model = $updateUseCase->handle(new UseCases\DTO\Update\Input($id, $data['name']));
