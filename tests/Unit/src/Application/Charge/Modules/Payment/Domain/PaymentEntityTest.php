...
namespace Tests\Unit\src\Application\Charge\Modules\Payment\Domain;
...
class PaymentEntityTest extends TestCase
{
    ...
    {
        $this->expectExceptionMessage('The payment is greater than the amount of the account payable');
    ...
    public function testCancelComplete()
    {
        ...
        $this->assertEquals('charge.payment.pay.remove.' . $objCharge->id(), $objCharge->events[0]->name());
        $this->assertEquals([
            'id' => $objCharge->id(),
            'value' => 40,
        ], $objCharge->events[0]->payload());
